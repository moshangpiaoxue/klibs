...
public class SimpleDate {
    ...
    public String getDisplayString(boolean hasYear) {
        if (mIsLunar) {
            return LunarCalendarUtil.getLunarStr(hasYear ? mDate[0] : 0, mDate[1], mDate[2]);
        }
    ...
    public static SimpleDate parseDateString(String date) {
        SimpleDate simpleDate = LunarCalendarUtil.parseNormalizedStr(date);
        if (simpleDate == null) {
            simpleDate = LunarCalendarUtil.parseLunarStr(date);
        }
        return simpleDate;
    }
