...
import mo.klib.utils.dataUtil.ChineseNumberUtil;

/**
 * @ author：mo
 * @ data：2019/2/13:14:13
 * @ 功能：农历日期相关
 */
public class LunarCalendarUtil {
        ...
        if (matcher.find()) {
            ...
            try {
                ...
                if (yearArStr != null) {
                    ...
                    year = ChineseNumberUtil.parseLunarYear(yearCnStr);
                }
                int month = ChineseNumberUtil.parseLunarMonth(monthStr);
                int day = ChineseNumberUtil.parseLunarDay(dayStr);
    ...
    public static String getLunarStr(int year, @IntRange(from = 1, to = 12) int month, @IntRange(from = 1, to = 31) int day) {
        ...
        builder.append(ChineseNumberUtil.formatLunarMonth(month)).append("月");
        builder.append(ChineseNumberUtil.formatLunarDay(day));
    ...
    public static String[] getLunarMonths(boolean hasMonthChar) {
        ...
        for (int i = 0; i < 12; i++) {
            if (hasMonthChar) {
                months[i] = ChineseNumberUtil.sCnMonths[i] + "月";
            } else {
                months[i] = String.valueOf(ChineseNumberUtil.sCnMonths[i]);
            }
        }
        return months;
    }
    ...
    public static String[] getLunarDays() {
        ...
        for (int i = 0; i < 9; i++) {
            days[i] = "初" + ChineseNumberUtil.sCnNums[i];
        }
        days[9] = "初十";
        for (int i = 10; i < 19; i++) {
            days[i] = "十" + ChineseNumberUtil.sCnNums[i - 10];
        }
        days[19] = "二十";
        for (int i = 20; i < 29; i++) {
            days[i] = "廿" + ChineseNumberUtil.sCnNums[i - 20];
        }
        days[29] = "三十";
        return days;
    }
