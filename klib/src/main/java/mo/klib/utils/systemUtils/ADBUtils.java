...
public class ADBUtils {
    ...
    public static boolean isRoot() {
        ShellUtil.CommandResult result = ShellUtil.execCmd("", true);
...
//    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult clearDataAndCache(String packageName) {
        return ShellUtil.execCmd("pm clear " + packageName, false);
    }
...
//    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult showRunningServices(String packageName) {
        return ShellUtil.execCmd("dumpsys activity services " + packageName, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult showAppDetailInfo(String packageName) {
        return ShellUtil.execCmd("dumpsys package " + packageName, true);
    }
     ...
     */
    public static ShellUtil.CommandResult showAppInstalledPath(String packageName) {
        return ShellUtil.execCmd("pm path " + packageName, false);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult startActivity(String packageName, String activityName) {
        return ShellUtil.execCmd(String.format("am start -n %s/%s", packageName, activityName), true);
    }
    ...
    public static ShellUtil.CommandResult startActivity(@NonNull Intent intent) {
        ...
        if (options.length() == 0) return new ShellUtil.CommandResult(-1, null, null);
        return ShellUtil.execCmd("am start" + options, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult startService(String packageName, String activityName) {
        return ShellUtil.execCmd(String.format("am startservice -n %s/%s ", packageName, activityName), true);
    }
    ...
    public static ShellUtil.CommandResult startService(@NonNull Intent intent) {
        String options = getIntentOptions(intent);
        if (options.length() == 0) return new ShellUtil.CommandResult(-1, null, null);
        return ShellUtil.execCmd("am startservice" + options, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult stopService(String packageName, String activityName) {
        return ShellUtil.execCmd(String.format("am stopservice -n %s/%s ", packageName, activityName), true);
    }
    ...
    public static ShellUtil.CommandResult stopService(@NonNull Intent intent) {
        String options = getIntentOptions(intent);
        if (options.length() == 0) return new ShellUtil.CommandResult(-1, null, null);
        return ShellUtil.execCmd("am stopservice" + options, true);
    }
    ...
    public static ShellUtil.CommandResult broadcast(@NonNull Intent intent) {
        String options = getIntentOptions(intent);
        if (options.length() == 0) return new ShellUtil.CommandResult(-1, null, null);
        return ShellUtil.execCmd("am broadcast" + options, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult forceStopApp(String packageName) {
        return ShellUtil.execCmd("am force-stop " + packageName, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult sendTrimMemory(String pid, String level) {
        return ShellUtil.execCmd(String.format("am send-trim-memory %s %s", pid, level), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult copyFile(String srcPath, String destPath) {
        return ShellUtil.execCmd(String.format("cp %s %s", srcPath, destPath), true);
    }
    ...
    */
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputKeyEvent(int keycode) {
        return ShellUtil.execCmd("input keyevent " + keycode, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputKeyEvent(String keycodeName) {
        return ShellUtil.execCmd("input keyevent " + keycodeName, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputKeyEvent(int keycode, boolean longPress) {
        String option = longPress ? "--longpress " : "";
        return ShellUtil.execCmd(String.format("input keyevent %s%s", option, keycode), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputText(String text) {
        return ShellUtil.execCmd("input text " + text, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputTab(int x, int y) {
        return ShellUtil.execCmd(String.format("input tap %s %s", x, y), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputSwipe(int x1, int y1, int x2, int y2) {
        return ShellUtil.execCmd(String.format("input swipe %s %s %s %s", x1, y1, x2, y2), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult inputSwipe(int x1, int y1, int x2, int y2, int duration) {
        return ShellUtil.execCmd(String.format("input swipe %s %s %s %s %s", x1, y1, x2, y2, duration), true);
    }
    ...
    public static ShellUtil.CommandResult showDeviceId() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return ShellUtil.execCmd("service call iphonesubinfo 1", true);
        } else {
            return ShellUtil.execCmd("dumpsys iphonesubinfo", true);
        }
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult showBuildProp(String prop) {
        return ShellUtil.execCmd("getprop " + prop, false);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult setScreenSize(int width, int height) {
        return ShellUtil.execCmd(String.format("wm size %sx%s", width, height), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult setScreenDensity(int density) {
        return ShellUtil.execCmd("wm density " + density, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult setScreenOverscan(int toLeft, int toTop, int toRight, int toBottom) {
        return ShellUtil.execCmd(String.format("wm overscan %s,%s,%s,%s", toLeft, toTop, toRight, toBottom), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult startAccessibilityService(String packageName, String serviceName) {
        return ShellUtil.execCmd(String.format("settings put secure enabled_accessibility_services %s/%s", packageName, serviceName), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult screenCapture(String path) {
        return ShellUtil.execCmd("screencap -p " + path, true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult screenRecord(String path, int timeLimit) {
        return ShellUtil.execCmd(String.format("screenrecord --time-limit %s %s", timeLimit, path), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult screenRecord(String path, int timeLimit, int width, int height) {
        return ShellUtil.execCmd(String.format("screenrecord --size %sx%s --time-limit %s %s", width, height, timeLimit, path), true);
    }
     ...
     * @return 命令执行结果
     */
    public static ShellUtil.CommandResult monkey(String packageName, int num) {
        return ShellUtil.execCmd(String.format("monkey -p %s -v %s", packageName, num), true);
    }
