...
public class CameraUtil2 {
    ...
    public static Uri actionMediaTakeVideo(Activity mActivity) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            videoUri = FileProvider.getUriForFile(mActivity, AppInfoUtil.getAppPackageName() + ".fileprovider", outVideo);
...
//            4.4(19)版本以下从相册选取图片后处理返回图片
            return CameraUtil2.getImagePath(data.getData(), null);
        }
    }
    ...
    public static String handlerImageAfter19(Intent data) {
        ...
        if (DocumentsContract.isDocumentUri(k.app(), uri)) {
            ...
            if ("com.android.providers.media.documents".equals(uri.getAuthority())) {
                ...
                imagePath = CameraUtil2.getImagePath(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, selection);
            } else if ("com.android.providers.downloads.documents".equals(uri.getAuthority())) {
                Uri contentUri = ContentUris.withAppendedId(Uri.parse("content://downloads/public_downloads"), Long.valueOf(docId));
                imagePath = CameraUtil2.getImagePath(contentUri, null);
            }
            //如果是content类型的URI，则使用普通方式处理
        } else if ("content".equalsIgnoreCase(uri.getScheme())) {
            imagePath = CameraUtil2.getImagePath(uri, null);
