...
public class ProcessUtil {
    ...
    public static String getForegroundProcessName() {
        ActivityManager manager =
                (ActivityManager) k.app().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            try {// 有"有权查看使用权限的应用"选项
                ApplicationInfo info =
                        packageManager.getApplicationInfo(k.app().getPackageName(), 0);
                AppOpsManager aom =
                        (AppOpsManager) k.app().getSystemService(Context.APP_OPS_SERVICE);
                if (aom != null) {
                    ...
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS,
                            info.uid,
                            info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        Log.i("ProcessUtil", "没有打开\"有权查看使用权限的应用\"选项");
                        return null;
                    }
                }
                UsageStatsManager usageStatsManager = (UsageStatsManager) k.app()
